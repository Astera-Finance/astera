...
interface IAERC6909 is IERC6909 {
    ...
    function initReserve(address underlyingAsset, string calldata name, string calldata symbol, uint8 decimals)
        external
    ...
    function transferFrom(address from, address to, uint256 id, uint256 amount) external override returns (bool);
    ...
    function getScaledUserBalanceAndSupply(address user, uint256 id) external view returns (uint256, uint256);
    ...
    function borrowAllowances(address delegator, address delegatee, uint256 id) external returns (uint256);
}
