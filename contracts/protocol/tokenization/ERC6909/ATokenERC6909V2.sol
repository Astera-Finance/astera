    ...
    IMiniPoolAddressProviderUpdatable
{
    ...
    function transferUnderlyingTo(address to, uint256 id, uint256 amount, bool unwrap) public {
        require(
            msg.sender == address(POOL) || msg.sender == _addressesProvider.getMainPoolAdmin(),
            Errors.AT_CALLER_MUST_BE_LENDING_POOL
        );
    ...
    function transferAllUnderlyingOut() external {
        if (_addressesProvider.getMainPoolAdmin() != msg.sender) {
            revert("V2: OnlyOwner");
        }

        for (uint256 id = ATOKEN_ADDRESSABLE_ID; _underlyingAssetAddresses[id] != address(0); id++)
        {
            address underlyingAsset = _underlyingAssetAddresses[id];
            uint256 amount = IERC20(underlyingAsset).balanceOf(address(this));
            transferUnderlyingTo(msg.sender, id, amount, true);
        }
        ILendingPool pool = ILendingPool(_addressesProvider.getLendingPool());
        for (
            uint256 id = ATOKEN_ADDRESSABLE_ID + pool.MAX_NUMBER_RESERVES();
            _underlyingAssetAddresses[id] != address(0);
            id++
        ) {
            address underlyingAsset = _underlyingAssetAddresses[id];
            uint256 amount = IERC20(underlyingAsset).balanceOf(address(this));
            transferUnderlyingTo(msg.sender, id, amount, false);
        }
    }
