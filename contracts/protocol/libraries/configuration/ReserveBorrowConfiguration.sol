...
library ReserveBorrowConfiguration {
    ...
    uint256 constant VOLATILITY_TIER_MASK =
        0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF8FFFFFFFFFFFFFFFFFFFFFFFF; // prettier-ignore
    ...
    function setLowVolatilityLtv(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 lowVolatilityLtv
    ) internal pure {
    ...
    function getLowVolatilityLtv(DataTypes.ReserveBorrowConfigurationMap storage self)
        internal
        view
        returns (uint256)
    {
    ...
    function setLowVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 threshold
    ) internal pure {
        require(threshold <= MAX_VALID_LIQUIDATION_THRESHOLD, Errors.RC_INVALID_LIQ_THRESHOLD);

        self.data = (self.data & LOW_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            | (threshold << LOW_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION);
    }

    function getLowVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap storage self
    ) internal view returns (uint256) {
        return (self.data & ~LOW_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            >> LOW_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION;
    }

    function setMediumVolatilityLtv(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 mediumVolatilityLtv
    ) internal pure {
        require(mediumVolatilityLtv <= MAX_VALID_LTV, Errors.RC_INVALID_LTV);

        self.data = (self.data & MEDIUM_VOLATILITY_LTV_MASK)
            | (mediumVolatilityLtv << MEDIUM_VOLATILITY_LTV_START_BIT_POSITION);
    }

    function getMediumVolatilityLtv(DataTypes.ReserveBorrowConfigurationMap storage self)
        internal
        view
        returns (uint256)
    {
    ...
    function setMediumVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 threshold
    ) internal pure {
        require(threshold <= MAX_VALID_LIQUIDATION_THRESHOLD, Errors.RC_INVALID_LIQ_THRESHOLD);

        self.data = (self.data & MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            | (threshold << MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION);
    }

    function getMediumVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap storage self
    ) internal view returns (uint256) {
        return (self.data & ~MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            >> MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION;
    }

    function setHighVolatilityLtv(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 highVolatilityLtv
    ) internal pure {
        require(highVolatilityLtv <= MAX_VALID_LTV, Errors.RC_INVALID_LTV);

        self.data = (self.data & HIGH_VOLATILITY_LTV_MASK)
            | (highVolatilityLtv << HIGH_VOLATILITY_LTV_START_BIT_POSITION);
    }

    function getHighVolatilityLtv(DataTypes.ReserveBorrowConfigurationMap storage self)
        internal
        view
        returns (uint256)
    {
    ...
    function setHighVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap memory self,
        uint256 threshold
    ) internal pure {
        require(threshold <= MAX_VALID_LIQUIDATION_THRESHOLD, Errors.RC_INVALID_LIQ_THRESHOLD);

        self.data = (self.data & HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            | (threshold << HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION);
    }

    function getHighVolatilityLiquidationThreshold(
        DataTypes.ReserveBorrowConfigurationMap storage self
    ) internal view returns (uint256) {
        return (self.data & ~HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
            >> HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION;
    }
    ...
    function setVolatilityTier(
        ...
        self.data = (self.data & VOLATILITY_TIER_MASK)
        ...
        returns (uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        return (
            ...
            (dataLocal & ~LOW_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
                >> LOW_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION,
            (dataLocal & ~MEDIUM_VOLATILITY_LTV_MASK) >> ~MEDIUM_VOLATILITY_LTV_START_BIT_POSITION,
            (dataLocal & ~MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
                >> MEDIUM_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION,
            (dataLocal & ~HIGH_VOLATILITY_LTV_MASK) >> ~HIGH_VOLATILITY_LTV_START_BIT_POSITION,
            (dataLocal & ~HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_MASK)
                >> HIGH_VOLATILITY_LIQUIDATION_THRESHOLD_START_BIT_POSITION
        );
    }
}
