 ...
 **/
contract MiniPoolCollateralManager is
  ...
  MiniPoolStorage
{
  ...
  function liquidationCall(
    ...
    DataTypes.MiniPoolReserveData storage collateralReserve = _reserves[collateralAsset];
    DataTypes.MiniPoolReserveData storage debtReserve = _reserves[debtAsset];
    ...
    (vars.errorCode, vars.errorMsg) = MiniPoolValidationLogic.validateLiquidationCall(
      collateralReserve,
      debtReserve,
      userConfig,
      vars.healthFactor,
    ...
    vars.userCollateralBalance = vars.collateralAtoken.balanceOf(user, vars.aTokenID);
    ...
    if (receiveAToken) {
      vars.liquidatorPreviousATokenBalance = vars.collateralAtoken.balanceOf(msg.sender, vars.aTokenID);
      vars.collateralAtoken.transferOnLiquidation(user, msg.sender, vars.aTokenID, vars.maxCollateralToLiquidate);
  ...
  function _calculateAvailableCollateralToLiquidate(
    ...
    if(collateralReserve.aTokenID % 1000 < 128){
        vars.collateralPrice = oracle.getAssetPrice(IAToken(collateralAsset).UNDERLYING_ASSET_ADDRESS());
    }else {
        vars.collateralPrice = oracle.getAssetPrice(collateralAsset);
    }
    if(debtReserve.variableDebtTokenID % 1000 < 128){
        vars.debtAssetPrice = oracle.getAssetPrice(IAToken(debtAsset).UNDERLYING_ASSET_ADDRESS());
    }else{
