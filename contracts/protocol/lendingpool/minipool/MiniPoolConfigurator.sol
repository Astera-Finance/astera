...
contract MiniPoolConfigurator is VersionedInitializable, IMiniPoolConfigurator {
   ...
   **/
  function batchInitReserve(InitReserveInput[] calldata input, IMiniPool pool) external onlyPoolAdmin {
  ...
  function _initReserve(IMiniPool pool, InitReserveInput calldata input) internal {
    ...
    (uint256 aTokenID, uint256 debtTokenID, bool isTranche) = 
    IAERC6909(AERC6909proxy).initReserve(
   ...
   * @param reserveType Whether the reserve is boosted by a vault
   **/
  function disableBorrowingOnReserve(address asset, bool reserveType, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
  ...
  function configureReserveAsCollateral(
    ...
    uint256 liquidationBonus, IMiniPool pool
   ...
   * @param reserveType Whether the reserve is boosted by a vault
   **/
  function activateReserve(address asset, bool reserveType, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
   ...
   * @param reserveType Whether the reserve is boosted by a vault
   **/
  function deactivateReserve(address asset, bool reserveType, IMiniPool pool) external onlyPoolAdmin {
    _checkNoLiquidity(asset, reserveType, pool);
   ...
   * @param reserveType Whether the reserve is boosted by a vault
   **/
  function freezeReserve(address asset, bool reserveType, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
   ...
   * @param reserveType Whether the reserve is boosted by a vault
   **/
  function unfreezeReserve(address asset, bool reserveType, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
   ...
   **/
  function setReserveFactor(address asset, bool reserveType, uint256 reserveFactor, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
  ...
  function setDepositCap(address asset, bool reserveType, uint256 depositCap, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset, reserveType);
  ...
  function setReserveVolatilityTier(address asset, bool reserveType, uint256 tier, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveBorrowConfigurationMap memory currentBorrowConfig = pool.getBorrowConfiguration(asset, reserveType);
    ...
    function setLowVolatilityLtv(address asset, bool reserveType, uint256 ltv, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveBorrowConfigurationMap memory currentBorrowConfig = pool.getBorrowConfiguration(asset, reserveType);
  ...
  function setMediumVolatilityLtv(address asset, bool reserveType, uint256 ltv, IMiniPool pool) external onlyPoolAdmin {
    DataTypes.ReserveBorrowConfigurationMap memory currentBorrowConfig = pool.getBorrowConfiguration(asset, reserveType);
  ...
  function setHighVolatilityLtv(address asset, bool reserveType, uint256 ltv, IMiniPool pool) external onlyPoolAdmin { // Store update params in array
    DataTypes.ReserveBorrowConfigurationMap memory currentBorrowConfig = pool.getBorrowConfiguration(asset, reserveType);
   ...
   **/
  function setReserveInterestRateStrategyAddress(address asset, bool reserveType, address rateStrategyAddress, IMiniPool pool)
    external
    onlyPoolAdmin
  {
