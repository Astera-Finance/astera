 ...
 library MiniPoolBorrowLogic {
  ...
  returns (
    ...
    for (vars.i = 0; vars.i < params.reservesCount; vars.i++) {
      ...
      DataTypes.MiniPoolReserveData storage currentReserve = reservesData[vars.currentReserveAddress];
      ...
      if (userConfig.isBorrowing(vars.i)) {
        vars.compoundedBorrowBalance = IAERC6909(currentReserve.aTokenAddress).balanceOf(params.user, currentReserve.variableDebtTokenID);
  ...
  returns (
    ...
    for (uint256 i; i < reservesCount; i++) {
      ...
      DataTypes.MiniPoolReserveData storage currentReserve = reservesData[currentReserveAddress];
  ...
  function executeBorrow(
    ...
    {//address oracle = vars.addressesProvider.getPriceOracle();
    uint256 amountInETH = amountInETH(vars.asset, vars.amount, reserve.configuration.getDecimals(), vars.addressesProvider.getPriceOracle());
    ...
    if (vars.releaseUnderlying) {
      IAERC6909(vars.aTokenAddress).transferUnderlyingTo(vars.user, reserve.aTokenID, vars.amount);
    }
