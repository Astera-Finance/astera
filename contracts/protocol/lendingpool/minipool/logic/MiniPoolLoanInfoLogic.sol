  ...
  ) external returns (bool)
  {
    ...
    if(params.updateLoan){
      ...
      for (vars.i; vars.i < length; vars.i++) {
        if(_userLoanInfo[params.user][vars.i].debtInfo.debtSnapshots[0].reserveID == getReserveIdByAddress(_reserves,params.debts[0])){
          if(!params.relationLoan){
            if(_userLoanInfo[params.user][vars.i].debtInfo.debtSnapshots[1].reserveID == getReserveIdByAddress(_reserves, params.debts[1])){
              vars.loanID = vars.i;
              break;
            }
          }
   ...
   //uint8[] memory rankedIDs = rankIDs(params.ltvs);
    ...
    for (vars.i; vars.i < length; vars.i++) {
      loanInfo.collateralInfo.collateralSnapshots[vars.i].reserveID = getReserveIdByAddress(_reserves, params.collaterals[vars.i]);
      loanInfo.collateralInfo.collateralSnapshots[vars.i].usedLTV = params.ltvs[vars.i];
      loanInfo.collateralInfo.collateralSnapshots[vars.i].usedLiquidationThreshold = params.liquidationThresholds[vars.i];
      loanInfo.collateralInfo.collateralSnapshots[vars.i].index = params.liquidityIndexes[vars.i];
      loanInfo.collateralInfo.collateralSnapshots[vars.i].amount = params.amountsCollateral[vars.i];
      loanInfo.collateralInfo.numCollateral = vars.i + 1;
      if(vars.i < params.debts.length){
        loanInfo.debtInfo.debtSnapshots[vars.i].reserveID = getReserveIdByAddress(_reserves, params.debts[vars.i]);
