...
describe("LendingPoolConfigurator", function () {
  ...
  it("Deactivates the ETH reserve", async function () {
    ...
    const { isActive } = await protocolDataProvider.getReserveConfigurationData(weth.address, false);
  ...
  it("Activates the ETH reserve for borrowing", async function () {
    ...
    const { variableBorrowIndex } = await protocolDataProvider.getReserveData(weth.address, false);
    const {
      decimals,
      ltv,
      liquidationBonus,
