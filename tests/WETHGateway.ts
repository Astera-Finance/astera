...
describe('WETHGateway', function () {
  ...
  it('withdrawETH', async function () {
    ...
    const withdrawalTx = await withdrawETH(
      wETHGateway,
      owner,
      lendingPoolProxy.address,
      false,
  ...
  it('borrowETH', async function () {
    ...
    const approveTx = await approveDelegation(
      variableDebtETH,
      owner,
      wETHGateway.address,
      ethMaxBorrowNative,
    ...
    const borrowTx = await borrowETH(
      wETHGateway,
      owner,
      lendingPoolProxy.address,
      false,
  ...
  it('repayETH', async function () {
    ...
    const repayTx = await repayETH(
      wETHGateway,
      owner,
      lendingPoolProxy.address,
      false,
      ethMaxBorrowNative.mul(2),
      owner.address,
    );
  ...
  it('emergencyEtherTransfer', async function () {
    ...
    const callTx = await destroyAndTransfer(
    ...
    ).to.equal(priorEthersBalance.sub(destoryGasCost).sub(transferGasCost));
    expect(await ethers.provider.getBalance(wETHGateway.address)).to.equal('0');
  });
});
