...
contract MiniPoolRepayWithdrawTransferTest is MiniPoolDepositBorrowTest {
    ...
    function testMiniPoolLiquidation(uint256 amount, uint256 collateralOffset, uint256 borrowOffset, uint256 priceDecrease) public {
        /**
         * Preconditions: 
        ...
        TokenParams memory collateralParams = TokenParams(erc20Tokens[collateralOffset], aTokens[collateralOffset], oracle.getAssetPrice(address(erc20Tokens[collateralOffset])));
        TokenParams memory borrowParams = TokenParams(erc20Tokens[borrowOffset], aTokens[borrowOffset], oracle.getAssetPrice(address(erc20Tokens[borrowOffset])));
        IAERC6909 aErc6909Token = IAERC6909(miniPoolContracts.miniPoolAddressesProvider.getMiniPoolToAERC6909(miniPool));

        /* Assumptions */
        amount = bound(amount, 10**(borrowParams.token.decimals()-2), borrowParams.token.balanceOf(address(this)) / 10);
...

        {
            ...
            miniPoolContracts.miniPoolAddressesProvider.setMiniPoolCollateralManager(miniPoolCollateralManager);
        }

        liquidationVars.collateralReserveDataBefore = IMiniPool(miniPool).getReserveData(address(collateralParams.token), true);
        liquidationVars.borrowReserveDataBefore = IMiniPool(miniPool).getReserveData(address(borrowParams.token), true);
        
        console.log("liquidityIndex: ", liquidationVars.borrowReserveDataBefore.liquidityIndex);
        console.log("variableBorrowIndex: ", liquidationVars.borrowReserveDataBefore.variableBorrowIndex);
        console.log("currentLiquidityRate: ", liquidationVars.borrowReserveDataBefore.currentLiquidityRate);
        console.log("availableMLPLiquidity: ", liquidationVars.borrowReserveDataBefore.availableMLPLiquidity); //@issue availableMLPLiquidity always 0
        console.log("currentVariableBorrowRate: ", liquidationVars.borrowReserveDataBefore.currentVariableBorrowRate);

        console.log("liquidityIndex: ", liquidationVars.collateralReserveDataBefore.liquidityIndex);
        console.log("variableBorrowIndex: ", liquidationVars.collateralReserveDataBefore.variableBorrowIndex);
        console.log("currentLiquidityRate: ", liquidationVars.collateralReserveDataBefore.currentLiquidityRate);
        console.log("availableMLPLiquidity: ", liquidationVars.collateralReserveDataBefore.availableMLPLiquidity); //@issue availableMLPLiquidity always 0
        console.log("currentVariableBorrowRate: ", liquidationVars.collateralReserveDataBefore.currentVariableBorrowRate);
        ...
        }
        {
            ...
            liquidationVars.userCollateralBalance = aErc6909Token.balanceOf(user, 1128 + collateralOffset);
            liquidationVars.liquidatorCollaterallBalance = collateralParams.token.balanceOf(liquidator);
            console.log("Mini pool aToken: ", address(aErc6909Token));
            console.log("Borrow ID: ", 2128 + borrowOffset);
            console.log("1. miniPool user debt token balance before: %s", liquidationVars.currentVariableDebt);
            console.log("1. miniPool user collateral balance before: %s", liquidationVars.userCollateralBalance);
            // console.log("?? lendingPool user balance before: %s", borrowParams.aToken.balanceOf(user));
            console.log("2. liquidator debt token balance before: %s", liquidationVars.liquidatorDebtTokenBalance);
            console.log("2. liquidator collateral balance before: %s", liquidationVars.liquidatorCollaterallBalance);

            vm.startPrank(liquidator);
            borrowParams.token.approve(miniPool, liquidationVars.amountToLiquidate);
            IMiniPool(miniPool).liquidationCall(
                address(collateralParams.token), true, address(borrowParams.token), true, user, liquidationVars.amountToLiquidate, false
            );
            console.log("--- aToken %s and token %s", address(borrowParams.aToken), address(borrowParams.token));
            console.log("--- aToken %s", address(aErc6909Token));
            console.log("1. miniPool user balance after: %s", aErc6909Token.balanceOf(user, 2128 + borrowOffset));
            console.log("2. miniPool user balance after: %s", aErc6909Token.balanceOf(user, 1128 + collateralOffset));

            console.log("3. lendingPool liquidator balance after: %s", collateralParams.token.balanceOf(liquidator));
            console.log("4. lendingPool liquidator balance after: %s", borrowParams.token.balanceOf(liquidator));

            console.log("User collateral token balance after liquidation shall be less or equal than user collateral balance before");
            assertLe(aErc6909Token.balanceOf(user, 1128 + collateralOffset), liquidationVars.userCollateralBalance/2);

            console.log("User debt token balance after liquidation shall be less by {amountToLiquidate} than user debt balance before");
            assertEq(liquidationVars.currentVariableDebt, aErc6909Token.balanceOf(user, 2128 + borrowOffset) + liquidationVars.amountToLiquidate);

            console.log("Liquidator collateral token balance after liquidation shall be greater or equal than half of user collateral balance + liquidator collateral balance before");
            assertGe(collateralParams.token.balanceOf(liquidator), liquidationVars.liquidatorCollaterallBalance + liquidationVars.userCollateralBalance/2);

            console.log("Liquidator debt token balance after liquidation shall be less by {amountToLiquidate} than user debt balance before");
            assertEq(liquidationVars.liquidatorDebtTokenBalance, aErc6909Token.balanceOf(liquidator, 2128 + borrowOffset) + liquidationVars.amountToLiquidate);

            (,,,,, uint256 healthFactorAfterLiquidation) = IMiniPool(miniPool).getUserAccountData(user);
            console.log("4. Health factor %s vs healthFactorAfterLiquidation %s: ", liquidationVars.healthFactor, healthFactorAfterLiquidation);
        ...
        {
            liquidationVars.collateralReserveDataAfter = IMiniPool(miniPool).getReserveData(address(collateralParams.token), true);
            liquidationVars.borrowReserveDataAfter = IMiniPool(miniPool).getReserveData(address(borrowParams.token), true);
            DataTypes.ReserveConfigurationMap memory configuration = IMiniPool(miniPool).getConfiguration(address(collateralParams.token), true);
            (,,liquidationVars.liquidationBonus,,) = configuration.getParamsMemory();
            liquidationVars.expectedCollateralLiquidated = borrowParams.price * (liquidationVars.amountToLiquidate * liquidationVars.liquidationBonus / 10_000)
                * 10 ** collateralParams.token.decimals() / (collateralParams.price * 10 ** borrowParams.token.decimals());
        }
        ...
        console.log("Debt before liquidation shall be greater by {amountToLiquidate=%s} than debt after liquidation", liquidationVars.amountToLiquidate);
        assertApproxEqRel(liquidationVars.currentVariableDebt, variableDebtBeforeTx - liquidationVars.amountToLiquidate, 0.01e18);
        ...
        //     0.01e18
        // );
        console.log("Liquidity index for debt token after liquidation shall be greater than liquidity index before liquidation");
        assertGe(liquidationVars.borrowReserveDataAfter.liquidityIndex, liquidationVars.borrowReserveDataBefore.liquidityIndex);
        console.log("Liquidity rate after liquidation shall be less than liquidity rate before liquidation");
        assertLt(liquidationVars.borrowReserveDataAfter.currentLiquidityRate, liquidationVars.borrowReserveDataBefore.currentLiquidityRate);
    ...
    function testLiquidationsWithFlowFromLendingPool(uint256 amount, uint256 collateralOffset, uint256 borrowOffset, uint256 skipDuration, uint256 priceDecrease) public 
    {
        collateralOffset = bound(collateralOffset, 0, tokens.length - 1);
        ...
        }
        {
            ...
            IMiniPool(miniPool).liquidationCall(
                address(collateralParams.token), true, address(borrowParams.token), true, user, amountToLiquidate, false
            );
            vm.stopPrank();
        }
        /**
