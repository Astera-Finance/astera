...
contract MiniPoolTest is Common {
    ...
    function setUp() public {
        ...
        inputs[1] = IMiniPoolConfigurator.InitReserveInput({
            ...
            interestRateStrategyAddress: configAddresses.stableStrategy,
            underlyingAsset: address(aTokens[0]),
            underlyingAssetName: aTokens[0].name(),
            underlyingAssetSymbol: aTokens[0].symbol()
        });
        assertEq(
        ...
        miniPoolContracts.miniPoolConfigurator.configureReserveAsCollateral(
            address(0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1), true, 9500, 9700, 10100, IMiniPool(mp)
        );

        miniPoolContracts.miniPoolConfigurator.configureReserveAsCollateral(
            address(aTokens[0]), true, 9500, 9700, 10100, IMiniPool(mp)
        );

        miniPoolContracts.miniPoolConfigurator.activateReserve(
            address(0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1), true, IMiniPool(mp)
        );

        miniPoolContracts.miniPoolConfigurator.activateReserve(address(aTokens[0]), true, IMiniPool(mp));

        miniPoolContracts.miniPoolConfigurator.enableBorrowingOnReserve(
            address(0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1), true, IMiniPool(mp)
        );

        miniPoolContracts.miniPoolConfigurator.enableBorrowingOnReserve(address(aTokens[0]), true, IMiniPool(mp));
        ...
        miniPoolContracts.miniPoolConfigurator.setReserveInterestRateStrategyAddress(
            address(0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1), true, address(IRS), IMiniPool(mp)
        );

        miniPoolContracts.miniPoolConfigurator.setReserveInterestRateStrategyAddress(
            address(aTokens[0]), true, address(IRS), IMiniPool(mp)
        );
    ...
    function testWithdrawalsZeroDebt() public {
        ...
        IMiniPool(mp).withdraw(address(aTokens[0]), false, 1e6, user);
        assertEq(aTokens[0].balanceOf(user), 1e6);
    }
    ...
    function testFlowLimiter() public {
        ...
        DataTypes.ReserveData memory reserveData =
            deployedContracts.lendingPool.getReserveData(address(vars.usdc), false);
        ...
        DataTypes.MiniPoolReserveData memory mpReserveData =
            IMiniPool(vars.mp).getReserveData(address(aTokens[0]), false);
    ...
    function testLiquidations() public {
        ...
        IMiniPool(vars.mp).liquidationCall(
            address(vars.dai), true, address(vars.grainUSDC), false, vars.user, type(uint256).max, true
        );
    }
}
