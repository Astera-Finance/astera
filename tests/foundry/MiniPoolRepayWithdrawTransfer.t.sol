...
contract MiniPoolRepayWithdrawTransferTest is MiniPoolDepositBorrowTest {
    ...
    function testWithdrawalsZeroDebt(uint256 amount, uint256 offset) public {
        ...
        console2.log("[Withdrawal]Offset: ", offset);
        ...
        console2.log("Balance of 6909 grain tokens: ", aErc6909Token.balanceOf(user, 1000));
        console2.log("Balance of 6909 tokens: ", aErc6909Token.balanceOf(user, 1128));

        uint256 usdcAmount = amount;
        console2.log(usdcAmount);
        uint256 grainUsdcAmount = amount;
        console2.log(grainUsdcAmount);
    ...
    function testMiniPoolNormalBorrowRepay(
        ...
        console2.log("Balance of total supply must be lower than before borrow");
        assertEq(
            aErc6909Token.scaledTotalSupply(2000 + borrowOffset), balances.totalSupply - amount
        );
        console2.log("Balance of AToken debt must be lower than before borrow");
        assertEq(aErc6909Token.balanceOf(user, 2000 + borrowOffset), balances.debtToken - amount);
        console2.log("Balance of AToken must be lower than before borrow");
        ...
        console2.log("total supply: ", balances.totalSupply);
        console2.log("debt token: ", balances.debtToken);
        console2.log("token: ", balances.token);
        console2.log("amount: ", amount);

        console2.log("Balance of total supply must be lower than before borrow");
        assertEq(aErc6909Token.scaledTotalSupply(2128 + borrowOffset), balances.debtToken - amount);
        console2.log("Balance of token debt must be lower than before borrow");
        assertEq(aErc6909Token.balanceOf(user, 2128 + borrowOffset), balances.totalSupply - amount);
        console2.log("Balance of token must be lower than before borrow");
    ...
    function testCannotWithdrawWhenBorrowedMaxLtv(
        ...
        // console2.log("collateralBalanceInUsd: ", reserveTokenParams.collateralBalanceInUsd);
        // console2.log("borrowBalanceInUsd: ", reserveTokenParams.borrowBalanceInUsd);
        ...
        vm.startPrank(user);
        console2.log("Withdraw function for token shall revert");
        ...
        IMiniPool(miniPool)
            .withdraw(address(collateralParams.token), false, balanceOfCollateral, user);
        // console2.log("Withdraw function for AToken shall revert");
        // vm.expectRevert(bytes(Errors.VL_TRANSFER_NOT_ALLOWED));
    ...
    function testWithdrawWhenBorrowed(
        ...
        console2.log("Withdraw token");
        IMiniPool(miniPool)
            .withdraw(address(collateralParams.token), false, withdrawAmount / 2, user);
        // console2.log("Withdraw AToken");
        // IMiniPool(miniPool).withdraw(address(collateralParams.aToken), true, withdrawAmount/2, user);

        {
            (,,,,, uint256 healthFactorAfter) = IMiniPool(miniPool).getUserAccountData(user);
            console2.log("healthFactor", healthFactorAfter);
    ...
    function testMultipleUsersBorrowRepayAndWithdraw(
        ...
        console2.log("Accrued wbtc debt: ", diff);
        ...
        console2.log("Accrued usdc debt: ", diff);
        ...
        wbtcParams.token
            .approve(address(miniPool), aErc6909Token.balanceOf(users.user1, 2128 + WBTC_OFFSET));
        ...
        usdcParams.token
            .approve(address(miniPool), aErc6909Token.balanceOf(users.user2, 2128 + USDC_OFFSET));
        ...
        vm.startPrank(users.user1);
        console2.log("Balance: ", aErc6909Token.balanceOf(users.user1, 1000 + USDC_OFFSET));
        console2.log("Balance: ", aErc6909Token.balanceOf(users.user1, 1128 + USDC_OFFSET));
        ...
        console2.log("Withdrawing... %s", aErc6909Token.balanceOf(users.user1, 1128 + USDC_OFFSET));
        IMiniPool(miniPool)
            .withdraw(
                address(usdcParams.token),
                false,
        ...
        console2.log("After Balance: ", aErc6909Token.balanceOf(users.user1, 1128 + USDC_OFFSET));
        availableLiquidity =
            IERC20(commonContracts.aTokensWrapper[USDC_OFFSET]).balanceOf(address(aErc6909Token));
        ...
        console2.log("Balance: ", aErc6909Token.balanceOf(users.user2, 1000 + WBTC_OFFSET));
        console2.log("Balance: ", aErc6909Token.balanceOf(users.user2, 1128 + WBTC_OFFSET));
        console2.log("AvailableLiquidity: ", availableLiquidity);
    ...
    // @issue11 Users are not able to withdraw all funds that they deposited after repaying
    function testMultipleUsersDepositBorrowRepayAndWithdrawWrapUnwrap(
        uint256 amount1,
        uint256 amount2,
        uint256 skipDuration
    ) public {
        ...
        {
            uint256 tokenId = 1128 + USDC_OFFSET;
            deal(address(usdcParams.token), users.user1, amount1);

            vm.startPrank(users.user1);
            uint256 tokenUserBalance = aErc6909Token.balanceOf(users.user1, tokenId);
            uint256 tokenBalance = usdcParams.token.balanceOf(users.user1);
            usdcParams.token.approve(address(miniPool), amount1);
            console2.log("User balance before: ", tokenBalance);
            IMiniPool(miniPool).deposit(address(usdcParams.token), true, amount1, users.user1);
            assertEq(
            ...
            assertApproxEqAbs(
                tokenUserBalance + amount1, aErc6909Token.balanceOf(users.user1, tokenId), 1
            );
            vm.stopPrank();
        }
        ...
        {
            uint256 tokenId = 1128 + WBTC_OFFSET;
            deal(address(wbtcParams.token), users.user2, amount2);

            vm.startPrank(users.user2);
            uint256 tokenUserBalance = aErc6909Token.balanceOf(users.user2, tokenId);
            uint256 tokenBalance = wbtcParams.token.balanceOf(users.user2);
            wbtcParams.token.approve(address(miniPool), amount2);
            console2.log("User balance before: ", tokenBalance);
            IMiniPool(miniPool).deposit(address(wbtcParams.token), true, amount2, users.user2);
            assertEq(
                tokenBalance - amount2,
                wbtcParams.token.balanceOf(users.user2),
                "wrong underlying balance"
            );
            assertApproxEqAbs(
                tokenUserBalance + amount2, aErc6909Token.balanceOf(users.user2, tokenId), 1
            );
            vm.stopPrank();
        }
    ...
    function testBorrowRepayAndWithdrawWithFlow(
        ...
        console2.log("----------------USER DEPOSITs LIQUIDITY (aUSDC) TO MINI POOL---------------");
        /* User deposits lending pool's aTokens to the mini pool and
        gets mini pool's aTokens */
        fixture_depositATokensToMiniPool(
        ...
        );
        {
            ...
            console2.log("Distributing borrowed asset to pay interests %s", diff);
            console2.log("----------------USER3---------------");
        ...
        wbtcParams.aToken
            .approve(address(miniPool), aErc6909Token.balanceOf(users.user1, 2000 + WBTC_OFFSET));
        console2.log(
            "1. Balance aToken: ", aErc6909Token.balanceOf(users.user1, 1000 + USDC_OFFSET)
        );
        console2.log(
        ...
        usdcParams.aToken
            .approve(address(miniPool), aErc6909Token.balanceOf(users.user2, 2000 + USDC_OFFSET));
        console2.log(
            "2. Normalized income USDC: ",
            ILendingPool(deployedContracts.lendingPool)
                .getReserveNormalizedIncome(address(usdcParams.token), true)
        );
        console2.log(
        ...
        uint256 availableLiquidity =
            IERC20(commonContracts.aTokens[USDC_OFFSET]).balanceOf(address(aErc6909Token));
        console2.log("2. AvailableLiquidity: ", availableLiquidity);
        console2.log("Provider repaying...");
        ...
        console2.log("Users2 debt: ", aErc6909Token.balanceOf(users.user2, 2000 + USDC_OFFSET));

        console2.log("----------------USER WITHDRAW---------------");
        vm.startPrank(users.user1);
        uint256 balanceAToken = aErc6909Token.balanceOf(users.user1, 1000 + USDC_OFFSET);
        console2.log("3. Balance aToken: ", balanceAToken);
        availableLiquidity =
            IERC20(commonContracts.aTokens[USDC_OFFSET]).balanceOf(address(aErc6909Token));
        // IERC20(reserve).balanceOf(aToken)
        console2.log("3. AvailableLiquidity: ", availableLiquidity);
        console2.log(
        ...
        console2.log("Withdrawing... %s", aErc6909Token.balanceOf(users.user1, 1000 + USDC_OFFSET));
        IMiniPool(miniPool)
            .withdraw(
                address(usdcParams.aToken),
                false,
        ...
        console2.log("After Balance: ", aErc6909Token.balanceOf(users.user1, 1000 + USDC_OFFSET));
        availableLiquidity =
            IERC20(commonContracts.aTokensWrapper[USDC_OFFSET]).balanceOf(address(aErc6909Token));
        console2.log("After availableLiquidity: ", availableLiquidity);
        assertEq(
    ...
    function testMiniPoolDepositAndWithdrawAfterSomeTime() public {
        ...
        console2.log("Balance before: ", tokenParamsUsdc.aToken.balanceOf(address(aErc6909Token)));
        console2.log("Balance to withdraw: ", aErc6909Token.balanceOf(user, 1000 + USDC_OFFSET));
        vm.startPrank(user);
        IMiniPool(miniPool)
            .withdraw(
        ...
        console2.log("Balance after: ", tokenParamsUsdc.aToken.balanceOf(address(aErc6909Token)));
    ...
    function testMinipoolIRStrategyOverflow() public {
        ...
        miniPoolContracts.miniPoolConfigurator
            .setFlowLimit(address(tokenParamsUsdc.token), 10000e6, IMiniPool(miniPool));
    ...
    function testMinipoolFlowBorrowTreasurySendATokenRemainder() public {
        ...
        vm.prank(miniPoolContracts.miniPoolAddressesProvider.getMainPoolAdmin());
        miniPoolContracts.miniPoolConfigurator.setAsteraTreasury(address(0x1111));
        ...
        address treasury = miniPoolContracts.miniPoolAddressesProvider.getMiniPoolAsteraTreasury();
        ...
        console2.log("treasuryBalance :: ", treasuryBalance);
    }
    ...
    function testMinipoolZigturH5_1() public {
        ...
        LendingPoolConfigurator(
            ...
            ).setAsteraReserveFactor(address(tokenParamsUsdc.token), true, 0);
        ...
        assertLe(
            ...
            AToken(address(tokenParamsUsdc.aToken))
                .convertToAssets(
                    IAERC6909(
    ...
    function testMinipoolZigturH5_2_1(uint256 seedTime, uint256 seedBr, uint256 seedLr) public {
        ...
        assertLe(
            FlowLimiter(miniPoolContracts.miniPoolAddressesProvider.getFlowLimiter())
                .currentFlow(address(tokenParamsUsdc.token), address(IMiniPool(miniPool))) - 1,
            AToken(address(tokenParamsUsdc.aToken))
                .convertToAssets(
                    IAERC6909(
                            miniPoolContracts.miniPoolAddressesProvider
    ...
    function testMinipoolZigturH5_2_2(
...

        {
            ...
            tokenParamsUsdc.aToken.approve(address(miniPool), balanceUsdcOwed);
            IMiniPool(miniPool)
                .repay(address(tokenParamsUsdc.aToken), false, balanceUsdcOwed, user2);
            vm.stopPrank();
    ...
    // Spearbit H3
    function testSpearbitH3_1() public {
        address user = makeAddr("user");
        address user2 = makeAddr("user2");
        ...
        vm.startPrank(user);
        deployedContracts.lendingPool
            .withdraw(address(tokenParamsUsdc.token), true, amountUsdc * 9 / 10, user);
        vm.stopPrank();
        ...
        vm.startPrank(user2);
        deployedContracts.lendingPool
            .withdraw(address(tokenParamsUsdc.token), true, amountUsdc * 9 / 10, user2);
        vm.stopPrank();
    ...
    function testSpearbitH3_2_minipoolFlowBorrowingCheck() public {
        uint256 BrAssetLp = 1e27;
        uint256 LrAssetLp = 1e26;
    ...
    function logInterestRate(address asset, address aToken) public view {
        console2.log(
            "Interest rate LP ::: %18e",
            deployedContracts.lendingPool.getReserveData(asset, true).currentVariableBorrowRate
        );

        console2.log(
            "Interest rate MP ::: %18e",
            IMiniPool(miniPool).getReserveData(aToken).currentLiquidityRate
        );
        console2.log("---");
    }
    ...
    function logMinipoolFlow(address asset, address aToken, address user) public view {
        ...
        uint256 liquidity1 = AToken(aToken)
            .convertToAssets(
                IAERC6909(
        ...
        console2.log("ERC20DebtTokens ::: %6e", flow);
        console2.log("ERC6909 AToken  ::: %6e", liquidity1);
        console2.logInt(int256(int256(liquidity1) - int256(flow)));
        console2.log("---");
    }
        ...
        public
    {
        ...
        balances.token = borrowParams.token.balanceOf(user);
        borrowParams.token.approve(address(miniPool), amount);
        console2.log("Repay 1");
        IMiniPool(miniPool).repay(address(borrowParams.token), false, amount, user);
        console2.log("Repay 2");
    ...
    function testDepositCapWithDeposit() public {
        ...
        vm.startPrank(miniPoolContracts.miniPoolAddressesProvider.getMainPoolAdmin());
        miniPoolContracts.miniPoolConfigurator
            .setDepositCap(address(erc20Tokens[WBTC_OFFSET]), 1, IMiniPool(miniPool));
        vm.stopPrank();
    ...
    function testDepositCapWithDeposit_18() public {
        ...
        vm.startPrank(miniPoolContracts.miniPoolAddressesProvider.getMainPoolAdmin());
        miniPoolContracts.miniPoolConfigurator
            .setDepositCap(address(erc20Tokens[WETH_OFFSET]), 1, IMiniPool(miniPool));
        vm.stopPrank();
        vm.prank(makeAddr("user"));
