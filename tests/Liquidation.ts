...
describe('Liquidations', function () {
  ...
  describe('Liquidates borrow for AToken', function () {
    it('Tries to liquidate healthy loan for AToken', async function () {
      ...
      const maxBitcoinBorrow = usdcMaxBorrowValue
        .div((await wbtcPriceFeed.latestAnswer()).div(ethers.utils.parseUnits('1', 8)));
      ...
      await expect(
        lendingPoolProxy.liquidationCall(
          usdc.address,
          false,
          wbtc.address,
          false,
    ...
    it('Liquidates unhealthy loan for AToken', async function () {
      ...
      expect(userAccountDataBefore.healthFactor).to.be.gt(
        MIN_HEALTH_FACTOR,
        'Starting health factor must be above 1',
      );
  ...
  describe('Liquidates borrow for Underlying', function () {
    ...
    it('Liquidates unhealthy loan for underlying', async function () {
      ...
      const usdcMaxBorrowValue = usdcDepositValue
        .mul(usdcLTV)
        .div(10000)
        .div(ethers.utils.parseUnits('1', 6));

      const maxBitcoinBorrow = usdcMaxBorrowValue
        .div((await wbtcPriceFeed.latestAnswer())
