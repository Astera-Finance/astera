...
describe('Pausable-Functions', function () {
  ...
  it('Liquidation call', async function () {
    ...
    const wethMaxBorrowValue = wethDepositValue
      .mul(wethLTV)
      .div(10000)
      .div(ethers.utils.parseEther('1'));
    const maxUsdcBorrow = wethMaxBorrowValue
      .div((await usdcPriceFeed.latestAnswer()).div(ethers.utils.parseUnits('1', 6)));

    await lendingPoolProxy.connect(borrower).borrow(
      usdc.address,
      false,
      maxUsdcBorrow,
      borrower.address,
    ...
    const newUsdcPriceFeed = await deployMockAggregator('120000000', usdc.decimals());
